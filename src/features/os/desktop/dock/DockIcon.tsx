...
import { COLOR_PALETTE, type AppID, useSystemStore } from "@/os/store";
...
export const DockIcon = memo(function DockIcon({
	...
	const colorRgb = COLOR_PALETTE[iconColor].value;
	// Derive gradient from the shared icon color (lighter → darker)
	const dynamicGradient: [string, string] = [`rgba(${colorRgb}, 0.8)`, `rgba(${colorRgb}, 0.5)`];
		...
		>
			...
			>
				{iconSrc ? (
					...
					<>
						{/* Gradient background — uses shared icon color from store */}
						<div
							className="absolute inset-0 shadow-lg"
							style={{
								background: `linear-gradient(145deg, ${dynamicGradient[0]}, ${dynamicGradient[1]})`,
								borderRadius: "22.5%",
								boxShadow: `
									0 2px 8px rgba(0,0,0,0.3),
									0 8px 24px rgba(0,0,0,0.2),
