...
function createTab(slug: string | null, label: string): Tab {
	return { id: `tab-${crypto.randomUUID()}`, slug, label };
}
				...
				{posts.map((post) => (
					...
					>
						...
						<span className="mt-3 inline-block text-xs font-medium text-blue-400">
							{post.pdfUrl ? "Open paper" : "Read more"}
						</span>
					</button>
				))}
			</div>
...
function BlogPostView({ post }: BlogPostViewProps) {
	...
	return (
		<article className={`mx-auto px-6 py-8 ${isPdf ? "max-w-5xl" : "max-w-2xl"}`}>
			<div className="flex flex-wrap items-center gap-2 text-xs text-white/40">
				<time>{formatDate(post.date)}</time>
				<span>·</span>
				<span>{getReadTime(post)}</span>
			...
			{post.tldr && (
				<p className="mt-4 text-sm italic leading-relaxed text-white/50">
					<span className="not-italic font-medium text-white/60">TL;DR: </span>
					{post.tldr}
				</p>
			)}
			...
			{post.pdfUrl && (
				<div className="mt-8">
					...
					<iframe
						...
						className="h-[min(80vh,900px)] w-full rounded-lg border border-white/10 bg-black/40"
					/>
				</div>
			)}
		</article>
