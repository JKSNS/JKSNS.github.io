...
const PROFILE_ACTIONS: ProfileAction[] = [
	{
		label: "Email",
		icon: Mail,
		href: "mailto:jackson@stephens.sh",
		ariaLabel: "Send email to Jackson",
	},
	{
		label: "GitHub",
		icon: Github,
		href: "https://github.com/JKSNS",
		ariaLabel: "View GitHub profile",
	},
	{
		label: "LinkedIn",
		icon: Linkedin,
		href: "https://www.linkedin.com/in/profile-jackson-stephens/",
...
function ActionButton({ action }: ActionButtonProps) {
	...
	const handleClick = useCallback(
		(e: React.MouseEvent<HTMLAnchorElement>) => {
			...
			// Framer-drag windows can swallow default <a> navigation; force external opens.
			if (!isMail) {
				e.preventDefault();
				window.open(action.href, "_blank", "noopener,noreferrer");
			}
		},
		[action.href, action.label, isMail],
	);
			...
			<div className="flex items-center gap-4 md:flex-col md:gap-4">
				...
				<div className="flex flex-col gap-1.5 md:items-center md:gap-2">
					...
					<div className="md:text-center">
						<h2 className="text-base font-semibold text-white">Jackson Stephens</h2>
						<p className="mt-0.5 text-xs text-white/50">Security Engineer & Researcher</p>
					</div>
